...
```sh
kubectl run hstream-admin -it --rm --restart=Never --image=hstreamdb/hstream:v0.12.0 -- \
...
```sh
kubectl run hstream-admin -it --rm --restart=Never --image=hstreamdb/hstream:v0.12.0 -- bash
```
